...
export default function CompanyHarborPage() {
  ...
  const minHealthBps = policy?.minHealthBps ?? 12500;
  const emergencyHealthBps = policy?.emergencyHealthBps ?? 11000;
  ...
  return (
    <div style={{ ...styles.page, paddingLeft: leftPad, transition: "padding-left 240ms cubic-bezier(0.4, 0, 0.2, 1)" }}>
      ...
      <HeaderStatusBar status={status} platform={platform} title="Harbor Health Systems" />
      <PlatformOverview status={status} platform={platform} />
      {error && <div style={styles.error}>Backend unreachable: {error}</div>}
      <div style={styles.layout}>
        <div style={styles.main}>
          <CompanyProfileCard
            name={status?.company?.name || "Harbor Health Systems"}
            riskProfile={status?.company?.riskProfile || "growth"}
        ...
        <div style={styles.side}>
          <CollateralPanel defaultUser="0x81008ADD908c9702FA595E942e8430AECEad8081" agentEnabled={status?.agentEnabled || false} snapshot={status?.snapshot || null} />
          <PaymentRequestForm defaultUser="0x81008ADD908c9702FA595E942e8430AECEad8081" />
        </div>
      </div>
    </div>
  );
