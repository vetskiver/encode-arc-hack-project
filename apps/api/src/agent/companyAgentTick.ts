 ...
 * but state is tracked per-company in the store (Upstash/Redis async).
 */
...
export async function companyAgentTick(companyId: string): Promise<void> {
  ...
  await store.updateCompanyTelemetry(companyId, { status: "Executing" });
  ...
  try {
    ...
    if (oracle.price <= 0) {
      await store.updateCompanyTelemetry(companyId, {
        status: "Risk Mode",
    ...
    await store.addCompanyPrice(companyId, oracle.price, oracle.ts);
    const changePct = await store.getCompanyChangePct(companyId);
    ...
    try {
      ...
      console.warn(
        `[CompanyTick:${companyId}] setOracleSnapshot failed (non-fatal):`,
        oracleErr?.message ?? oracleErr
      );
    }
    ...
    const maxBorrowUSDC =
      collateralValueUSDC * (company.policy.ltvBps / 10000);

    const debtUSDC = company.debtUSDC;
    const healthFactor = debtUSDC > 0.01 ? maxBorrowUSDC / debtUSDC : 999;

    const totalUSDC =
      company.liquidityUSDC + company.reserveUSDC + company.yieldUSDC;

    const liquidityRatio =
      totalUSDC > 0 ? company.liquidityUSDC / totalUSDC : 0;

    const reserveRatioActual =
      totalUSDC > 0 ? company.reserveUSDC / totalUSDC : 0;

    const pending = await store.getCompanyPendingPayment(companyId);

    const p = company.policy;

    const dailyRemaining = await store.getCompanyDailyRemaining(companyId);
    ...
    const snapshot: Snapshot = {
      ...
      // On-chain style integers (as your original code did)
      ...
      // These are floats (as in your original code)
      ...
      pendingPayment: pending
        ? {
            to: pending.to,
            // pending.amountUSDC is likely a string in your store; keep same behavior:
            amountUSDC: parseFloat(pending.amountUSDC),
          }
        : null,
    ...
    // Save snapshot to telemetry (UI-friendly)
    const snapshotForUI = {
      oraclePrice: oracle.price,
      oracleTs: oracle.ts,
      oracleStale: oracle.stale,
    ...
    await store.updateCompanyTelemetry(companyId, { lastSnapshot: snapshotForUI });
    ...
    if (!safetyResult.allowed) {
      ...
      await store.addCompanyLog(companyId, {
        ts: Date.now(),
        action: "BLOCKED",
        amountUSDC: "0",
        healthFactor,
        ...
        circleTxRef: "",
        arcTxHash: "",
        trigger: `price=${oracle.price.toFixed(4)}, vol=${Math.abs(changePct).toFixed(
          1
    ...
    // 6) Post-execution: recompute HF with fresh company state
    const postCompany = await store.getCompany(companyId);
    if (!postCompany) return;

    const postCollValue = postCompany.collateralUnits * oracle.price;
    const postMaxBorrow = postCollValue * (p.ltvBps / 10000);
    const postHF =
      postCompany.debtUSDC > 0.01 ? postMaxBorrow / postCompany.debtUSDC : 999;

    // Update telemetry snapshot with post values
    await store.updateCompanyTelemetry(companyId, {
      lastSnapshot: {
        ...snapshotForUI,
        ...
        liquidityRatio: (
          postCompany.liquidityUSDC +
            postCompany.reserveUSDC +
            postCompany.yieldUSDC >
          0
            ? postCompany.liquidityUSDC /
              (postCompany.liquidityUSDC +
                postCompany.reserveUSDC +
    ...
    const preIsRisk =
      healthFactor < minHealth || Math.abs(changePct) > p.volatilityThresholdPct;

    if (preIsRisk) {
      await store.updateCompanyTelemetry(companyId, {
        status: "Risk Mode",
        lastReason: `Risk: HF ${healthFactor.toFixed(
          2
        )} → ${postHF.toFixed(2)}, vol=${changePct.toFixed(1)}%, executed ${
          safetyResult.plan.actions.length
        } action(s)`,
      });
      ...
      await store.updateCompanyTelemetry(companyId, {
        status: "Monitoring",
        lastReason: `All healthy. HF=${postHF.toFixed(
          2
        )}, liq=$${postCompany.liquidityUSDC.toFixed(2)}`,
      });
    }

    console.log(
      `[CompanyTick:${companyId}] Done. HF=${postHF.toFixed(2)}, actions=${
        safetyResult.plan.actions.length
      }`
    );
  } catch (err: any) {
    console.error(`[CompanyTick:${companyId}] Error:`, err?.message ?? err);
    await store.updateCompanyTelemetry(companyId, {
      status: "Risk Mode",
      lastReason: `Error: ${err?.message ?? String(err)}`,
    });
  }
}
...
async function executeCompanyAction(
  ...
  let circleTxRef = `sim-${companyId}-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 8)}`;
  ...
  switch (action.type) {
    case "borrow": {
      ...
      try {
        ...
        console.warn(
          `[CompanyTick:${companyId}] Circle borrow failed (using sim ref):`,
      ...
      try {
        ...
        console.warn(
          `[CompanyTick:${companyId}] Arc recordBorrow failed (non-fatal):`,
          arcErr?.message ?? arcErr
        );
      }
      break;
    }
    ...
    case "repay": {
      ...
      // Real Circle: liquidity/reserve -> creditFacility
      try {
        ...
        console.warn(
          `[CompanyTick:${companyId}] Circle repay failed (using sim ref):`,
          circleErr?.message ?? circleErr
        );
      }
      ...
      try {
        arcTxHash = await arc.recordRepay(
          companyAddress,
          BigInt(Math.round(actualRepay * 1e6)),
          circleTxRef
        );
        console.log(`[CompanyTick:${companyId}] Arc recordRepay: ${arcTxHash}`);
      } catch (arcErr: any) {
        console.warn(
          `[CompanyTick:${companyId}] Arc recordRepay failed (non-fatal):`,
          arcErr?.message ?? arcErr
        );
      }
      break;
    ...
    case "rebalance": {
      ...
      try {
        ...
        console.warn(
          `[CompanyTick:${companyId}] Circle rebalance failed (using sim ref):`,
          circleErr?.message ?? circleErr
        );
      }
      ...
      try {
        arcTxHash = await arc.recordRebalance(
          from,
          to,
          BigInt(Math.round(transferAmt * 1e6)),
          circleTxRef
        );
        console.log(`[CompanyTick:${companyId}] Arc recordRebalance: ${arcTxHash}`);
      } catch (arcErr: any) {
        console.warn(
          `[CompanyTick:${companyId}] Arc recordRebalance failed (non-fatal):`,
          arcErr?.message ?? arcErr
        );
      }
      break;
    ...
    case "payment": {
      ...
      try {
        ...
        circleTxRef = result.circleTxRef;
        console.log(
          `[CompanyTick:${companyId}] Circle payment tx: ${circleTxRef} -> ${recipient}`
        );
      } catch (circleErr: any) {
        console.warn(
          `[CompanyTick:${companyId}] Circle payment failed (using sim ref):`,
          circleErr?.message ?? circleErr
        );
      }
      ...
      await store.removeCompanyPendingPayment(companyId);

      // Real Arc: record payment on-chain
      try {
        arcTxHash = await arc.recordPayment(
          companyAddress,
          recipient,
          BigInt(Math.round(payAmt * 1e6)),
          circleTxRef
        );
        console.log(`[CompanyTick:${companyId}] Arc recordPayment: ${arcTxHash}`);
      } catch (arcErr: any) {
        console.warn(
          `[CompanyTick:${companyId}] Arc recordPayment failed (non-fatal):`,
          arcErr?.message ?? arcErr
        );
      }
      break;
  ...
  if (action.type === "borrow" || action.type === "payment") {
    await store.recordCompanyDailySpend(companyId, amount);
  }

  // Log decision on Arc with full context (non-fatal)
  try {
    ...
    const decisionHash = await arc.logDecision(
      snapshotStr,
      `${action.type}:${amount.toFixed(2)}`,
      rHash
    );
    ...
    console.warn(
      `[CompanyTick:${companyId}] Arc logDecision failed (non-fatal):`,
      arcErr?.message ?? arcErr
    );
  }
  ...
  const log: ActionLog = {
    ...
    trigger:
      action.trigger ||
      `price=${snapshot.oraclePrice.toFixed(4)}, vol=${snapshot.volatilityPct.toFixed(
        1
  ...
  await store.addCompanyLog(companyId, log);
}

function getBucketBalance(company: CompanyProfile, bucket: string): number {
  switch (bucket) {
    case "liquidity":
      return company.liquidityUSDC;
    case "reserve":
      return company.reserveUSDC;
    case "yield":
      return company.yieldUSDC;
    default:
      return 0;
  }
}
...
function subtractBucket(company: CompanyProfile, bucket: string, amount: number): void {
  switch (bucket) {
    case "liquidity":
      company.liquidityUSDC = Math.max(0, company.liquidityUSDC - amount);
      break;
    case "reserve":
      company.reserveUSDC = Math.max(0, company.reserveUSDC - amount);
      break;
    case "yield":
      company.yieldUSDC = Math.max(0, company.yieldUSDC - amount);
      break;
  }
}
...
function addBucket(company: CompanyProfile, bucket: string, amount: number): void {
  switch (bucket) {
    case "liquidity":
      ...
      break;
    case "reserve":
      company.reserveUSDC += amount;
      break;
    case "yield":
      company.yieldUSDC += amount;
      break;
  }
}